...
class ContextSettingsWidget(QtGui.QWidget, ContextViewMixin):
    ...
    schema_dict = {
        ...
        "package_filter": Or(
            And(None, Use(lambda x: [])), And(dict, Use(lambda x: [x])), [dict]
        ),
        "caching": bool,
    }
    ...
    def __init__(self, context_model=None, attributes=None, parent=None):
        ...
        schema_dict = dict(
            (k, v) for k, v in self.schema_dict.items() if k in self.schema_keys
        )
    ...
    def apply_changes(self):
        def _content_error(title, text):
            ret = QtGui.QMessageBox.warning(
                self, title, text, QtGui.QMessageBox.Discard, QtGui.QMessageBox.Cancel
            )
