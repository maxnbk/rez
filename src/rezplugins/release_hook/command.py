...
class CommandReleaseHook(ReleaseHook):
    ...
    commands_schema = Schema(
        {
            "command": basestring,
            Optional("args"): Or(
                And(basestring, Use(lambda x: x.strip().split())), [basestring]
            ),
    ...
    def pre_build(self, user, install_path, variants=None, **kwargs):
        ...
        self._execute_commands(
            self.settings.pre_build_commands,
    ...
    def pre_release(self, user, install_path, variants=None, **kwargs):
        errors = []
        self._execute_commands(
            self.settings.pre_release_commands,
            install_path=install_path,
            package=self.package,
            errors=errors,
            variants=variants,
    ...
    def post_release(self, user, install_path, variants, **kwargs):
        ...
        self._execute_commands(
            self.settings.post_release_commands,
            install_path=install_path,
            package=package,
            errors=errors,
            variants=variants,
    ...
    def _execute_commands(
        self, commands, install_path, package, errors=None, variants=None
    ):
        release_dict = dict(path=install_path)
        variant_infos = []
        if variants:
            for variant in variants:
                if isinstance(variant, (int, int)):
