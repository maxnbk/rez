...
def setup_parser(parser, completions=False):
    parser.add_argument("-l", "--list", action="store_true", help="list visible suites")
    parser.add_argument(
        "-t",
        "--tools",
        dest="print_tools",
    ...
    parser.add_argument("--validate", action="store_true", help="validate the suite")
    parser.add_argument(
        "--create", action="store_true", help="create an empty suite at DIR"
    )
    parser.add_argument(
        "-c",
        "--context",
    ...
    add_action = parser.add_argument(
        "-a", "--add", type=str, metavar="RXT", help="add a context to the suite"
    )
    add_action = parser.add_argument(
        "-P",
        "--prefix-char",
    ...
    parser.add_argument(
        "-p", "--prefix", type=str, help="set the prefix of a context in the suite"
    )
    parser.add_argument(
        "-s", "--suffix", type=str, help="set the suffix of a context in the suite"
    )
    parser.add_argument(
        "--hide", type=str, metavar="TOOL", help="hide a tool of a context in the suite"
    )
    parser.add_argument(
        "--unhide",
        type=str,
    ...
    DIR_action = parser.add_argument(
        "DIR", type=str, nargs="?", help="directory of suite to create or manage"
    )
...
def command(opts, parser, extra_arg_groups=None):
    ...
    context_needed = set(
        ("add", "prefix", "suffix", "hide", "unhide", "alias", "unalias", "interactive")
    )
    save_needed = set(
        (
            "add",
    ...
    def _option(name):
        ...
        if value and name in context_needed and not opts.context:
            parser.error(
                "--context must be supplied when using --%s" % name.replace("_", "-")
            )
        return value
    ...
    if _option("interactive"):
        ...
        context_names = suite.find_contexts(
            in_request=opts.find_request, in_resolve=opts.find_resolve
        )
        if context_names:
            print("\n".join(context_names))
        ...
        suite.add_context(
            name=opts.context, context=context, prefix_char=opts.prefix_char
        )
    elif _option("remove"):
        ...
        _pr(
            "aliasing tool %r as %r in context %r..."
            % (opts.alias[0], opts.alias[1], opts.context)
        )
        suite.alias_tool(
            context_name=opts.context, tool_name=opts.alias[0], tool_alias=opts.alias[1]
        )
