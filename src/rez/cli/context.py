...
def setup_parser(parser, completions=False):
    ...
    parser.add_argument(
        ...
        dest="show_uris",
        action="store_true",
        help="list resolved package's URIs, rather than the default 'root' " "filepath",
    )
    parser.add_argument(
        "-t",
        "--tools",
    ...
    parser.add_argument(
        "--which", type=str, metavar="CMD", help="locate a program within the context"
    )
    parser.add_argument(
        "-g",
        "--graph",
...
def command(opts, parser, extra_arg_groups=None):
    ...
    if opts.format in ("dict", "table", "json"):
        ...
        code = rc.get_shell_code(
            shell=opts.format, parent_environ=parent_env, style=OutputStyle[opts.style]
        )
        print(code)
