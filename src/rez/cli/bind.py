...
def setup_parser(parser, completions=False):
    ...
    parser.add_argument(
        "-l", "--list", action="store_true", help="list all available bind modules"
    )
    parser.add_argument(
        "-s",
        "--search",
    ...
    parser.add_argument("PKG", type=str, nargs="?", help="package to bind")
    parser.add_argument(
        "BIND_ARGS",
        metavar="ARG",
...
def command(opts, parser, extra_arg_groups=None):
    ...
    if opts.quickstart:
        ...
        for name in names:
            ...
            variants_ = bind_package(name, path=install_path, no_deps=True, quiet=True)
