...
class Wrapper(object):
    ...
    def __init__(self, filepath):
        ...
        def _err(msg):
            raise RezSystemError("Invalid executable file %s: %s" % (filepath, msg))
    ...
    def _run(self, prefix_char, args):
        ...
        parser = argparse.ArgumentParser(prog=self.tool_name, prefix_chars=prefix_char)
        ...
        if opts.patch is not None:
            ...
            context = ResolvedContext(
                request, package_paths=pkg_paths, verbosity=opts.verbose
            )
        ...
        retcode, _, _ = context.execute_shell(
            command=cmd, stdin=opts.stdin, quiet=opts.quiet, block=True
        )
        return retcode
    ...
    def peek(self):
        config.remove_override("quiet")
        new_context = ResolvedContext(
            self.context.requested_packages(), package_paths=self.context.package_paths
        )
